...
#[derive(Debug)]
enum TicketNewError {
    TitleError { msg: String },
    DescriptionError { msg: String },
}
...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description.clone(), status.clone()) {
        Ok(ticket) => ticket,
        Err(ticket_error) => match ticket_error {
            TicketNewError::TitleError { msg } => {
                panic!("{}", msg);
            }
            TicketNewError::DescriptionError { msg } => Ticket::new(
                title.clone(),
...
impl Ticket {
    pub fn new(
        ...
        if title.len() > 50 {
            return Err(TicketNewError::TitleError {
                msg: "Title cannot be longer than 50 bytes".to_string(),
            });
        }
        ...
        if description.len() > 500 {
            return Err(TicketNewError::DescriptionError {
                msg: "Description cannot be longer than 500 bytes".to_string(),
            });
        }
